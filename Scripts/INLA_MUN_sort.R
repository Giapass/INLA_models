...
#Spatial
mesh = readRDS('./Data/mesh_spring_1.rds')#Mesh from previous steps
mesh$n
...
#Temporal
t<-11
mesh.t1 = inla.mesh.1d(1:11)
h.spec = list(theta=list(prior='pc.cor1', param=c(0, 0.9)))#hyperprior for AR1

#Index space-time
index.st <- inla.spde.make.index('i', n.spde = mesh$n, n.group = 11)
...
mesh.loc =cbind(rep(mesh$loc[,1],11),rep(mesh$loc[,2],11),sort(rep(seq(1:11),mesh$n)))
#Time for the mesh
time_mesh<-sort(rep(seq(1:11),mesh$n))
...
r.ar1 <- inla(f.ar,family='binomial', data = inla.stack.data(stk.all.ar),#stk.dat.ar
             control.predictor = list(A = inla.stack.A(stk.all.ar),link=1, compute= TRUE),
             ...
             num.threads = 5,control.compute = list(openmp.strategy="huge"))#number of threads are the core of the machine you can adjust
#if you are using pardiso  control.compute = list(openmp.strategy = ”pardiso.parallel”)
#next try with restart=3, stupid.search.max.iter=100

#5. Plots######
par(mfrow = c(4, 2), mar = c(3, 3.5, 0, 0), mgp = c(1.5, 0.5, 0), las = 0) 
plot(r.ar1$marginals.fix[[1]], type = 'l', #only 75 values in marginal fixed always
     xlab = 'Intercept', ylab = 'Density') 
plot(r.ar1$marginals.fix[[2]], type = 'l', 
     xlab = 'Ocean distance coefficient', ylab = 'Density') 
plot(r.ar1$marginals.fix[[3]], type = 'l', 
     xlab = 'Batimetry  coefficient', ylab = 'Density') 
plot(r.ar1$marginals.fix[[4]], type = 'l', 
     xlab = 'SST  coefficient', ylab = 'Density') 
plot(r.ar1$marginals.fix[[5]], type = 'l', 
     xlab = 'Salinity coefficient', ylab = 'Density') 
plot(r.ar1$marginals.fix[[6]], type = 'l', 
     xlab = 'Chl', ylab = 'Density') 
plot(r.ar1$marginals.fix[[7]], type = 'l', 
     xlab = 'GSST', ylab = 'Density') 
par(mfrow = c(3, 1), mar = c(3, 3.5, 0, 0), mgp = c(1.5, 0.5, 0), las = 0) 
for (j in 1:3) 
  plot(r.ar1$marginals.hy[[j]], type = 'l', 
       ylab = 'Density', xlab = names(r.ar1$marginals.hyperpar)[j])

#5.2.Prediction on a grid
...
poly.only<- rgeos::gDifference(pl.grid, peru_extend)
...
mu.st1 <- lapply(1:mesh$n, function(j) {
  ...
  r <- inla.mesh.project(projgrid,field = r.ar1$summary.ran$i$mean[idx])
  r[!xy.in] <- NA
  return(r)
})
...
index_predic <- inla.stack.index(stk.all.ar,"pred_1")$data
stpred <- matrix(r.ar1$summary.fitted.values$mean[index_predic], 
                 spde$n.spde)
