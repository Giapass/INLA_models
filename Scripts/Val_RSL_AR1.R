...
data_pred = data_1p[,6:11]

#Convert to UTMs
x_loc2 = cbind(data_1p$Lon_M, data_1p$Lat_M)
spdat2 <- data_1p
coordinates(spdat2) <- ~ Lon_M + Lat_M
proj4string(spdat2) <- "+init=epsg:4326"
kmproj2 <- CRS("+proj=utm +zone=18S ellps=WGS84 +units=km")
x_loc_utm2 <- spTransform(spdat2, kmproj2)
x_loc_utm2<-cbind(x_loc_utm2@coords[,1],x_loc_utm2@coords[,2])

#3.1 Projector matrix####
data_1p$Year<-as.numeric(factor(data_1p$Year))

Aprd<- inla.spde.make.A(mesh=mesh, loc=x_loc_utm2, group.mesh=mesh.t1,
                        group = data_1p$Year)
...
saveRDS(r.ar1,'val.ar_summer_0.rds')
