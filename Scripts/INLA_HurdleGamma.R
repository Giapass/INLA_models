...
data06_13$DCsc<-as.numeric(scale(data06_13$DC,center=TRUE))
set.seed(123)
sample_size = floor(0.1*nrow(data06_13))
...
dim(Aspte)==c(length(data06_13t$time),(t*spde$n.spde))#to confirm the correct dim of matrix A
#4.Stack data##############
#4.1 Make index
...
stk.z.est <- inla.stack(data = list(Y = cbind(data06_13t$z,NA),link=1),#NEW 
                         ...
                         effects = list(index.stz, 
                                        data.frame(z.Intercept=rep(1,length(data06_13t$DC)),
                                                   oceanDist_z= data06_13t$DCsc)),tag = 'zdat') 
stk.y.est <- inla.stack(data = list(Y = cbind(NA,data06_13t$y),link=2),#NEW 
                        A = list(Aspte,1),
                        effects = list(c(index.stc,index.sty), 
                                       data.frame(y.Intercept = rep(1,length(data06_13t$DC)),
                                                  oceanDist_y = data06_13t$DCsc)),tag = 'ydat') 
...
DCmesh <-apply(spDists(shoreline_utm,meshcoord,longlat = FALSE), 2, min)#only run the first time
DCmeshsc <-rep(as.numeric(scale(DCmesh)),t)
...
stk.z.prd <- inla.stack(data = list(Y = matrix(NA, ncol(Aprd), 2), link = 1),
                         A = list(Aprd,1), 
                         effects = list(index.stz,
                                        data.frame(z.Intercept=rep(1,ncol(Aprd)),oceanDist_z=DCmeshsc)),tag = 'zprd') 
stk.y.prd <- inla.stack(data = list(Y = matrix(NA, ncol(Aprd), 2), link = 2),
                         A = list(Aprd,1), 
                         effects = list(c(index.stc,index.sty),
                                        data.frame(y.Intercept=rep(1,ncol(Aprd)),oceanDist_y=DCmeshsc)),tag = 'yprd') 
#4.4 Stack all
...
r.DCjoint <- inla(f.DCjoint, data = inla.stack.data(stk.all.hurdle), 
                  ...
                  control.inla = list(strategy = 'adaptive', int.strategy = 'eb'),num.threads=2,verbose=TRUE)
summary(r.DCjoint)
#Plot the posterior
